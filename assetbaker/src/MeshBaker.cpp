...
namespace RSim::AssetBaker {
    ...
    MeshBaker::MeshBaker(std::filesystem::path const &FilePath,
        ...
        ManualTimer timer;
        aiScene const *pScene = importer.ReadFile(FilePath.string(),
        ...
        for (size_t i = 0; i < pScene->mNumMeshes; ++i) {
            ...
            for (size_t FaceIndex = 0; FaceIndex < pMesh->mNumFaces; ++FaceIndex) {
                ...
                InitIndicesFromFace(Indices, face, FaceIndex);
            }
            ...
            for (size_t VertexIndex = 0; VertexIndex < pMesh->mNumVertices; ++VertexIndex) {
                ...
                aiVector3D const *pUV = pMesh->mTextureCoords[0];
                aiVector3D UV{0.0f, 0.0f, 0.0f};

                aiColor4D const *pColor = pMesh->mColors[0];
                ...
                InitVertexFromAssimp(Vertex, Position, Normal, *(aiColor3D*)(&Color) ,UV);
                Vertices[VertexIndex] = Vertex;
            }
            AssetLib::MeshInfo Info = CreateMeshInfo(Vertices, Indices, FilePath.string());
            AssetLib::Asset asset = PackMesh(Info, reinterpret_cast<char *>(Vertices.data()),
            ...
            if (SaveBinaryFile(FinalOutputPath, asset)) {
                ...
                baker_info("The mesh data was compressed by {0:.3f}%.",
                           100.0f - (float) CompressedSize / (float) OriginalSize * 100.0f);
    ...
    void MeshBaker::InitIndicesFromFace(std::vector<uint32_t> &indices, aiFace const &pFace, size_t BaseFaceIndex) {
    ...
    void MeshBaker::InitVertexFromAssimp(AssetLib::Vertex_F32PNCV &Vertex,
